...
2. Selecciona build con Dockerfile y usa `services/logto-invitations/Dockerfile` como ruta.
3. Configura `services/logto-invitations` como Docker Context Path.
4. Configura las variables de `.env.example` en la aplicacion.
5. Asigna `invites-betipo-tools.monteserin.dev` al puerto interno `3000`, con HTTPS.
6. Despliega y comprueba `/health`.
